...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-50 text-slate-900 max-w-md mx-auto shadow-xl relative">
      ...
      {/* 
        Fix: TypeScript error because currentView is narrowed by the exclusion check.
        We cast to string to allow the comparison for the scanner button's styling.
      */}
      {state.currentView !== 'player' && state.currentView !== 'scanner' && (
        <nav className="h-20 bg-white/80 backdrop-blur-lg border-t flex items-center justify-around px-2 sticky bottom-0 z-50 pb-safe">
          ...
          <button 
            ...
            className={`flex flex-col items-center gap-1 transition-colors ${(state.currentView as string) === 'scanner' ? 'text-blue-600' : 'text-slate-400'}`}
          >
